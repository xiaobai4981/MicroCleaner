package com.leyou.microstorageanalyzer.ui.explorer
...
import com.leyou.microstorageanalyzer.databinding.ItemFileExplorerBinding
import com.leyou.microstorageanalyzer.global.utils.NumberUtil
            ...
            views.textViewDescription.text = when (data.type) {
                ...
                FileTreeInfo.FileType.ANDROID -> {
                    ApplicationHolder.app.packageManager.getPackageArchiveInfo(data.path, PackageManager.GET_ACTIVITIES)?.let { packageInfo: PackageInfo ->
                        val appName = packageInfo.applicationInfo.apply { this?.publicSourceDir = data.path }?.loadLabel(ApplicationHolder.app.packageManager)
